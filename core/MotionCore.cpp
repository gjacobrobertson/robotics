...
void MotionCore::loadCalibration() {
  ...
  std::string calibrationFilename = boost::lexical_cast<std::string>(config.robot_id) + "_calibration.cal";
  std::string file = memory_.data_path_ + calibrationFilename;
  RobotCalibration cal;
  printf("Loading calibration from file: %s\n", file.c_str());
  bool loaded = cal.loadFromFile(file);
  if(loaded) {
...
void MotionCore::preProcess() {
  ...
  if(calibration_ && calibration_->enabled) {
    calibration_->applyJoints(processed_joint_angles_->values_);
  }
}
