...
ParticleFilter::ParticleFilter(MemoryCache& cache, TextLogger*& tlogger) 
  : cache_(cache), tlogger_(tlogger), reseed_factor(0.9), dirty_(true) {
}
...
void ParticleFilter::reseed() {
  ...
  for (int i=100*reseed_factor; i < particles().size(); i++) {
...
void ParticleFilter::processFrame() {
  ...
  double sum_w = 0;
  for(auto& p: particles()) {
//    bool binomial = rand_.sampleB(0.99);
//    if (binomial) {
...
/*    } else {
  ...
  //Resample
  int n = particles().size() * reseed_factor;
...
const Pose2D& ParticleFilter::pose() const {
  ...
  if(dirty_) {
    ...
    for(const auto& p : particles()) {
      if (i < reseed_factor*particles().size()) {
    ...
    if(particles().size() > 0)
      mean_ /= (particles().size()*reseed_factor);
    dirty_ = false;
  }
  return mean_;
}
