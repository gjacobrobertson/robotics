...
#    commands.setStiffness()
    ...
    eta = float('inf')
    print "Ball: {0}, {1} Velocity: {2}, {3} Vision: {4} {5}".format(ball.x, ball.y, ball.absVel.x, ball.absVel.y, ball.visionDistance, ball.visionBearing*core.RAD_T_DEG)
    if ball.absVel.x < 0:
    ...
    if eta < 10 and relBall.x < 1000 and eta > 3:
      intercept = relBall.y + ball.relVel.y * eta
      ...
      target_pos = get_target_position(ball)
  ...
  def get_target_position(self, ball):
   ...
   def move_to_position(self, target_x, target_y)
      ...
      if abs(angleToTarget) > 15 * core.DEG_T_RAD:
        tVel = 0.5 * (angleToTarget / abs(angleToTarget))
      
      xToTarget = target_x - robot.loc.x
      if abs(xToTarget) > 100:
        xVel = 0.5 * (xToTarget / abs(xToTarget))

      yToTarget = target_y - robot.loc.y
      if abs(yToTarget) > 100:
        yVel = 0.5 * (yToTarget / abs(yToTarget))
